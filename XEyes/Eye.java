...
public class Eye {
    ...
    public boolean inEye(){
        if (Math.abs(eyeCenterX-eyeX) < 0.5*eyeWidth && Math.abs(eyeCenterY-eyeY) < 0.5*eyeHeight)
            System.out.println("hi");
        return (Math.abs(eyeCenterX-eyeX) < 0.5*eyeWidth && Math.abs(eyeCenterY-eyeY) < 0.5*eyeHeight);
    ...
    public void cEye (Graphics g, int cx, int cy){
        int centerX = (int) (cx + (0.5 * 49));
        int centerY = (int) (cy + (0.5 * 37));
        g.setColor(Color.black);
        g.fillOval (cx, cy, eyeWidth, eyeHeight);

        g.setColor(Color.white);
        g.fillOval (cx + eyeballBorder, cy + eyeballBorder, 
                    eyeWidth - (2 * eyeballBorder), eyeHeight - (2 * eyeballBorder));
                    
        g.setColor(Color.black);
        g.fillOval(centerX - pupilRadius, centerY - pupilRadius, 2 * pupilRadius, 2 * pupilRadius);
    }
